...
function XMLImportSurvey($sFullFilePath, $sXMLdata = null, $sNewSurveyName = null, $iDesiredSurveyId = null, $bTranslateInsertansTags = true, $bConvertInvalidQuestionCodes = true)
{
    ...
    foreach ($xml->surveys_languagesettings->rows->row as $row) {
        ...
        if ($bTranslateInsertansTags) {
            ...
            if (isset($insertdata['surveyls_ccodepo'])) {
                $insertdata['surveyls_ccodepo'] = translateLinks('survey', $iOldSID, $iNewSID, $insertdata['surveyls_ccodepo']);
            }
            if (isset($insertdata['surveyls_nsize'])) {
                $insertdata['surveyls_nsize'] = translateLinks('survey', $iOldSID, $iNewSID, $insertdata['surveyls_nsize']);
            }
            ...
            if (isset($insertdata['surveyls_ccodepo']) && checkOldLinks('survey', $iOldSID, $insertdata['surveyls_ccodepo'])) {
                $results['importwarnings'][] = gT("Survey Case Code/ PO has outdated links.");
            }
            if (isset($insertdata['surveyls_nsize']) && checkOldLinks('survey', $iOldSID, $insertdata['surveyls_nsize'])) {
                $results['importwarnings'][] = gT("Survey N Size has outdated links.");
            }
