...
class SurveyAdministrationController extends LSBaseController
{
    ...
    public function actionInsert()
    {
        if (Permission::model()->hasGlobalPermission('surveys', 'create')) {
            ...
            $surveyCcodepo = Yii::app()->request->getPost('surveyls_ccodepo');
            $surveyNsize = Yii::app()->request->getPost('surveyls_nsize');
            $surveyTitle = trim((string) $surveyTitle);
            $surveyCcodepo = trim((string) $surveyCcodepo);
            $surveyNsize = trim((integer) $surveyNsize);
            if ($surveyTitle == '') {
            ...
            if ($surveyCcodepo == '') {
                $alertError = gT("Survey could not be created because it did not have a Case Code/ PO#");
            ...
            if ($surveyNsize == '') {
                $alertError = gT("Survey could not be created because it did not have a N Size");
            ...
            $simpleSurveyValues->ccodepo = $surveyCcodepo;
            $simpleSurveyValues->nsize = $surveyNsize;
            ...
            if ($createSample) {
                ...
                //after create survey this view is displayed
    ...
    public function actionGetCurrentEditorValues($sid)
    {
        ...
        if ($oSurvey == null) {
            ...
            $aReturner["surveyCcodepo"][$defaultLanguage] = "";
            $aReturner["surveyNsize"][$defaultLanguage] = "";
        ...
        foreach ($oSurvey->allLanguages as $sLanguage) {
            ...
            $aReturner["surveyCcodepo"][$sLanguage] = $oSurvey->languagesettings[$sLanguage]->surveyls_ccodepo;
            $aReturner["surveyNsize"][$sLanguage] = $oSurvey->languagesettings[$sLanguage]->surveyls_nsize;
            $aReturner["welcome"][$sLanguage] = $oSurvey->languagesettings[$sLanguage]->surveyls_welcometext;
            $aReturner["ccodepo"][$sLanguage] = $oSurvey->languagesettings[$sLanguage]->surveyls_ccodepo;
            $aReturner["nsize"][$sLanguage] = $oSurvey->languagesettings[$sLanguage]->surveyls_nsize;
    ...
    public function actionSaveTextData($sid)
    {
        ...
        foreach ($changes as $sLanguage => $contentChange) {
            ...
            $oSurveyLanguageSetting->surveyls_ccodepo = $contentChange['ccodepo'];
            $oSurveyLanguageSetting->surveyls_nsize = $contentChange['nsize'];
