...
class CreateSurvey
{
    ...
    private function createRelationSurveyLanguageSettings($langsettings)
    {
        ...
        $sCcodepo = html_entity_decode($this->simpleSurveyValues->ccodepo, ENT_QUOTES, "UTF-8");

        // Fix bug with FCKEditor saving strange BR types
        $sCcodepo = fixCKeditorText($sCcodepo);

        $sNsize = html_entity_decode($this->simpleSurveyValues->nsize, ENT_QUOTES, "UTF-8");

        // Fix bug with FCKEditor saving strange BR types
        $sNsize = fixCKeditorText($sNsize);
        ...
        $aInsertData = array(
            ...
            'surveyls_ccodepo' => $sCcodepo,
            'surveyls_nsize' => $sNsize,
